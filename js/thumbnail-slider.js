function ThumbnailSlider(e) {
	...
	M.prototype = {
		...
		m: function () {
			for (var e = u(d), c = 0, f = e[g]; c < f; c++)
				if (e[c].i == a.a) break;
				else d[s](e[c]);
			if (h) d[j].left = n - b[a.a][l] + "px";
			else if (E) d[j].top = n - b[a.a][k] + "px";
		...
		s: function () {
			a.f = (h ? d[l] : d[k]) < 0;
			if (a.f) J(p, "navPrevDisabled");
			else L(p, "navPrevDisabled");
			a.e = (h ? d[l] - f[A] : d[k] - f[t]) + B > 0;
			if (a.e) J(r, "navNextDisabled");
			else L(r, "navNextDisabled")
		}
	};
	var I = function () {
		var a = document.getElementById(e.sliderId);
		if (a && u(a)[g] && a[t]) y = new M(a);
